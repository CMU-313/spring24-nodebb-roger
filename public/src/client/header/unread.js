	...
	unread.initUnreadTopics = function () {
		...
		function onNewPost(data) {
			if (data && data.posts && data.posts.length > 0 && unreadTopics) {
				...
				const isNewTopic = post.isMain && Number.parseInt(post.uid, 10) !== Number.parseInt(app.user.uid, 10);
				if (isNewTopic && !unreadTopics.new[tid]) {
					increaseUnreadCount('new');
					unreadTopics.new[tid] = true;
				}

				const isUnreplied = Number.parseInt(post.topic.postcount, 10) <= 1;
		...
		function increaseUnreadCount(filter) {
			...
			const newCount = 1 + Number.parseInt($('a[href="' + config.relative_path + unreadUrl + '"].navigation-link i').attr('data-content'), 10);
