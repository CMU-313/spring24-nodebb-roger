...
], (header, translator, zxcvbn, api, alerts) => {
	...
	function handlePasswordChange() {
		...
		$('#changePasswordBtn').on('click', function () {
			...
			if (passwordvalid && passwordsmatch) {
				button.addClass('disabled').find('i').removeClass('hide');
				api.put('/users/' + ajaxify.data.theirid + '/password', {
					currentPassword: currentPassword.val(),
					newPassword: password.val(),
				})
					.then(() => {
						if (Number.parseInt(app.user.uid, 10) === Number.parseInt(ajaxify.data.uid, 10)) {
					...
					.finally(() => {
						button.removeClass('disabled').find('i').addClass('hide');
						currentPassword.val('');
						password.val('');
