...
define('admin/advanced/errors', ['bootbox', 'alerts', 'Chart'], (bootbox, alerts, Chart) => {
	const Errors = {};
	...
	Errors.clear404 = function () {
		bootbox.confirm('[[admin/advanced/errors:clear404-confirm]]', ok => {
			if (ok) {
				socket.emit('admin.errors.clear', {}, error => {
					if (error) {
	...
	Errors.setupCharts = function () {
		const notFoundCanvas = document.querySelector('#not-found');
		const tooBusyCanvas = document.querySelector('#toobusy');
