...
define('admin/modules/dashboard-line-graph', ['Chart', 'translator', 'benchpress', 'api', 'hooks', 'bootbox'], (Chart, translator, Benchpress, api, hooks, bootbox) => {
	const Graph = {
		_current: null,
	};
	let isMobile = false;
	...
	Graph.init = ({set, dataset}) => {
		const canvas = document.querySelector('#analytics-traffic');
		...
		return new Promise(resolve => {
			t.translateKey(`admin/menu:${ajaxify.data.template.name.replace('admin/', '')}`, []).then(key => {
				...
				Graph._current = new Chart(canvasContext, {
					type: 'line',
					data,
					options: {
						responsive: true,
	...
	Graph.update = (
		...
		return new Promise(resolve => {
			api.get(`/admin/analytics/${set}`, {units, until, amount}).then(dataset => {
				Graph._current.data.xLabels = units === 'days' ? utils.getDaysArray(until, amount) : utils.getHoursArray();
				...
				apiElement.attr('href', `${config.relative_path}/api/v3/admin/analytics/${ajaxify.data.set}?${newHref}`);
