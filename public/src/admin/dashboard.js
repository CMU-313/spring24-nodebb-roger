...
define('admin/dashboard', [
	...
	Admin.updateRoomUsage = function (error, data) {
		if (error) {
	...
	function setupGraphs(callback) {
		callback ||= function () {};
		const trafficCanvas = document.querySelector('#analytics-traffic');
		const registeredCanvas = document.querySelector('#analytics-registered');
		const presenceCanvas = document.querySelector('#analytics-presence');
		const topicsCanvas = document.querySelector('#analytics-topics');
		const trafficContext = trafficCanvas.getContext('2d');
		const registeredContext = registeredCanvas.getContext('2d');
		const presenceContext = presenceCanvas.getContext('2d');
		const topicsContext = topicsCanvas.getContext('2d');
		...
		Promise.all([
			...
			graphs.traffic = new Chart(trafficContext, {
				type: 'line',
				data,
				options: {
					responsive: true,
			...
			graphs.registered = new Chart(registeredContext, {
				type: 'doughnut',
				data: {
			...
			graphs.presence = new Chart(presenceContext, {
				type: 'doughnut',
				data: {
			...
			graphs.topics = new Chart(topicsContext, {
				type: 'doughnut',
				data: {
					labels: [],
					datasets: [{
			...
			$('[data-action="updateGraph"]:not([data-units="custom"])').on('click', function () {
				...
				require(['translator'], translator => {
					translator.translate('[[admin/dashboard:page-views-custom]]', translated => {
			...
			$('[data-action="updateGraph"][data-units="custom"]').on('click', function () {
				...
				Benchpress.render('admin/partials/pageviews-range-select', {}).then(html => {
					const modal = bootbox.dialog({
						...
						modal.find('#startRange').val(targetElement.attr('data-startRange') || yesterday);
						modal.find('#endRange').val(targetElement.attr('data-endRange') || today);
					});
					...
					function submit() {
						...
						if (!validRegexp.test(formData.startRange) || !validRegexp.test(formData.endRange)) {
							// Invalid Input
						...
						targetElement.attr('data-startRange', formData.startRange);
						targetElement.attr('data-endRange', formData.endRange);
						targetElement.html(formData.startRange + ' &ndash; ' + formData.endRange);
					}
				});
			});
	...
	function updateTrafficGraph(units, until, amount) {
		...
		socket.emit('admin.analytics.get', {
			...
			const apiElement = $('#view-as-json');
			const newHref = $.param({
				units: units || 'hours',
				until,
				count: amount,
			});
			apiElement.attr('href', config.relative_path + '/api/admin/analytics?' + newHref);
		});
	}
	...
	function updateTopicsGraph(topics) {
		if (topics.length === 0) {
			translator.translate('[[admin/dashboard:no-users-browsing]]', translated => {
				topics = [{
					title: translated,
					count: 1,
				}];
	...
	function setupFullscreen() {
		...
		const button = $container.find('.fa-expand');
		let fsMethod;
		let exitMethod;
