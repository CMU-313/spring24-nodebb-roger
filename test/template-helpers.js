...
describe('helpers', () => {
	...
	it('should generate topic class', done => {
		const className = helpers.generateTopicClass({
			locked: true, pinned: true, deleted: true, unread: true,
		});
	...
	it('should show leave button if isMember and group is not administrators', done => {
		const button = helpers.membershipBtn({displayName: 'some group', name: 'some group', isMember: true});
		assert.equal(button, '<button class="btn btn-danger" data-action="leave" data-group="some group"><i class="fa fa-times"></i> [[groups:membership.leave-group]]</button>');
		done();
	});

	it('should show pending button if isPending and group is not administrators', done => {
		const button = helpers.membershipBtn({displayName: 'some group', name: 'some group', isPending: true});
		assert.equal(button, '<button class="btn btn-warning disabled"><i class="fa fa-clock-o"></i> [[groups:membership.invitation-pending]]</button>');
		done();
	});

	it('should show reject invite button if isInvited', done => {
		const button = helpers.membershipBtn({displayName: 'some group', name: 'some group', isInvited: true});
		assert.equal(button, '<button class="btn btn-link" data-action="rejectInvite" data-group="some group">[[groups:membership.reject]]</button><button class="btn btn-success" data-action="acceptInvite" data-group="some group"><i class="fa fa-plus"></i> [[groups:membership.accept-invitation]]</button>');
		done();
	});

	it('should show join button if join requests are not disabled and group is not administrators', done => {
		const button = helpers.membershipBtn({displayName: 'some group', name: 'some group', disableJoinRequests: false});
		assert.equal(button, '<button class="btn btn-success" data-action="join" data-group="some group"><i class="fa fa-plus"></i> [[groups:membership.join-group]]</button>');
		done();
	});

	it('should show nothing if group is administrators ', done => {
		const button = helpers.membershipBtn({displayName: 'administrators', name: 'administrators'});
		assert.equal(button, '');
		done();
	});
