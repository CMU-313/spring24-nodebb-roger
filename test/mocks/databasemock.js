...
const databaseType = nconf.get('database');
const testDatabaseConfig = nconf.get('test_database');
const productionDatabaseConfig = nconf.get(databaseType);
...
if (testDatabaseConfig.database === productionDatabaseConfig.database
    && testDatabaseConfig.host === productionDatabaseConfig.host
    && testDatabaseConfig.port === productionDatabaseConfig.port) {
	const errorText = 'test_database has the same config as production db';
	winston.error(errorText);
	throw new Error(errorText);
}

nconf.set(databaseType, testDatabaseConfig);

winston.info('database config %s', databaseType, testDatabaseConfig);
