...
describe('Flags', () => {
	...
	describe('.list()', () => {
		it('should show a list of flags (with one item)', done => {
			Flags.list({
				...
				Flags.get(payload.flags[0].flagId, (error, flagData) => {
					assert.ifError(error);
		...
		describe('(with filters)', () => {
			it('should return a filtered list of flags if said filters are passed in', done => {
				Flags.list({
					...
					assert.ok(payload.hasOwnProperty('pageCount'));
					assert.ok(Array.isArray(payload.flags));
					assert.strictEqual(1, Number.parseInt(payload.flags[0].flagId, 10));
					done();
				});
			});
		...
		describe('(with sort)', () => {
			...
			it('should return sorted flags latest first if no sort is passed in', async () => {
				...
				assert(payload.flags.every((current, index) => {
					if (index === payload.flags.length - 1) {
						return true;
					}

					const next = payload.flags[index + 1];
					return Number.parseInt(current.datetime, 10) > Number.parseInt(next.datetime, 10);
				}));
			});
			...
			it('should return flags with more reports first if "reports" sort is passed in', async () => {
				...
				assert(payload.flags.every((current, index) => {
					...
					const next = payload.flags[index + 1];
					return Number.parseInt(current.heat, 10) >= Number.parseInt(next.heat, 10);
				}));
			});
		});
	});
	...
	describe('.update()', () => {
		it('should alter a flag\'s various attributes and persist them to the database', done => {
			Flags.update(1, adminUid, {
				...
				db.getObjectFields('flag:1', ['state', 'assignee'], (error, data) => {
					...
					assert.ok(!isNaN(Number.parseInt(data.assignee, 10)));
					assert.strictEqual(adminUid, Number.parseInt(data.assignee, 10));
					done();
				});
			});
		});
		...
		it('should allow assignment if user is an admin and do nothing otherwise', async () => {
			...
			assert.strictEqual(adminUid, Number.parseInt(assignee, 10));
		...
		it('should allow assignment if user is a global mod and do nothing otherwise', async () => {
			...
			let assignee = await db.getObjectField('flag:1', 'assignee');
			assert.strictEqual(uid3, Number.parseInt(assignee, 10));
		...
		describe('resolve/reject', () => {
			...
			it('should rescind notification if flag is resolved', async () => {
				...
				await Flags.update(flagObject.flagId, adminUid, {
					state: 'resolved',
				});
	...
	describe('(v3 API)', () => {
		...
		describe('.appendNote()', () => {
			it('should append a note to the flag', async () => {
				...
				assert.strictEqual(1, Object.keys(response.history.at(-1).fields).length);
				assert(response.history.at(-1).fields.hasOwnProperty('notes'));
			});
		});
