...
describe('Post\'s', () => {
	...
	before(done => {
		async.series({
			...
			globalModuleUid = results.globalModUid;
			...
			topics.post({
				...
				groups.join('Global Moderators', globalModuleUid, done);
			});
		});
	});
	...
	it('should update category teaser properly', async () => {
		...
		const postResult = await topics.post({
			uid: globalModuleUid, cid, title: 'topic title', content: '123456789',
		});
		...
		const newPostResult = await topics.post({
			uid: newUid, cid, title: 'topic title', content: 'xxxxxxxx',
		});
	...
	it('should change owner of post and topic properly', async () => {
		...
		const postResult = await topics.post({
			uid: oldUid, cid, title: 'change owner', content: 'original post',
		});
	...
	it('should return falsy if post does not exist', done => {
		posts.getPostData(9999, (error, postData) => {
	...
	describe('voting', () => {
		...
		it('should get voters', done => {
			socketPosts.getVoters({uid: globalModuleUid}, {pid: postData.pid, cid}, (error, data) => {
				assert.ifError(error);
		...
		it('should get upvoters', done => {
			socketPosts.getUpvoters({uid: globalModuleUid}, [postData.pid], (error, data) => {
				assert.ifError(error);
		...
		it('should prevent downvoting more than total daily limit', async () => {
			...
			assert.equal(error.message, '[[error:too-many-downvotes-today, 1]]');
		...
		it('should prevent downvoting target user more than total daily limit', async () => {
			...
			assert.equal(error.message, '[[error:too-many-downvotes-today-user, 1]]');
	...
	describe('post tools', () => {
		it('should error if data is invalid', done => {
			socketPosts.loadPostTools({uid: globalModuleUid}, null, error => {
		...
		it('should load post tools', done => {
			socketPosts.loadPostTools({uid: globalModuleUid}, {pid: postData.pid, cid}, (error, data) => {
				assert.ifError(error);
        ...
        */
		...
		it('topic owner can see pin button', done => {
			const topicOwnerUid = topicData.uid;
			socketPosts.loadPostTools({uid: topicOwnerUid}, {pid: postData.pid, cid}, (error, data) => {
				assert.ifError(error);
		...
		it('admin can see the pin button', done => {
			socketPosts.loadPostTools({uid: adminUid}, {pid: postData.pid, cid}, (error, data) => {
				assert.ifError(error);
				assert(data.posts.displayPin);
				done();
			});
		...
		it('random user cannot see the pin button', done => {
			socketPosts.loadPostTools({uid: randomUserUid}, {pid: postData.pid, cid}, (error, data) => {
				assert.ifError(error);
				assert(!(data.posts.displayPin));
				done();
			});
	...
	describe('delete/restore/purge', () => {
		...
		it('should error with invalid data', async () => {
			try {
				...
				return assert.equal(error.message, '[[error:invalid-data]]');
			}
		...
		it('should not see post content if global mod does not have posts:view_deleted privilege', done => {
			async.waterfall([
				...
				function (next) {
					helpers.loginUser('global mod', '123456', (error, data) => {
						assert.ifError(error);
						request(`${nconf.get('url')}/api/topic/${tid}`, {jar: data.jar, json: true}, (error, res, body) => {
							assert.ifError(error);
		...
		it('should delete topic if last main post is deleted', async () => {
			const data = await topics.post({
				uid: voterUid, cid, title: 'test topic', content: 'test topic',
			});
			await apiPosts.delete({uid: globalModuleUid}, {pid: data.postData.pid});
	...
	describe('edit', () => {
		...
		it('should error if user is not logged in', async () => {
			try {
				...
				return assert.equal(error.message, '[[error:not-logged-in]]');
			}
		...
		it('should error if title is too short', async () => {
			try {
				...
				return assert.equal(error.message, `[[error:title-too-short, ${meta.config.minimumTitleLength}]]`);
			}
		...
		it('should error if title is too long', async () => {
			...
			try {
				...
				return assert.equal(error.message, `[[error:title-too-long, ${meta.config.maximumTitleLength}]]`);
			}
		...
		it('should error with too many tags', async () => {
			...
			try {
				...
				return assert.equal(error.message, `[[error:too-many-tags, ${meta.config.maximumTagsPerTopic}]]`);
			}
		...
		it('should error if content is too short', async () => {
			try {
				...
				return assert.equal(error.message, `[[error:content-too-short, ${meta.config.minimumPostLength}]]`);
			}
		...
		it('should error if content is too long', async () => {
			...
			try {
				...
				return assert.equal(error.message, `[[error:content-too-long, ${meta.config.maximumPostLength}]]`);
			}
		...
		it('should disallow post editing for new users if post was made past the threshold for editing', async () => {
			...
			try {
				await apiPosts.edit({uid: voterUid}, {
					pid, content: 'edited post content again', title: 'edited title again', tags: ['edited-twice'],
				});
			} catch (error) {
				assert.equal(error.message, '[[error:post-edit-duration-expired, 1]]');
		...
		it('should edit a deleted post', async () => {
			...
			const data = await apiPosts.edit({uid: voterUid}, {
				pid, content: 'edited deleted content', title: 'edited deleted title', tags: ['deleted'],
			});
		...
		it('should return diffs', done => {
			posts.diffs.get(replyPid, 0, (error, data) => {
				assert.ifError(error);
		...
		it('should load diffs and reconstruct post', done => {
			posts.diffs.load(replyPid, 0, voterUid, (error, data) => {
				assert.ifError(error);
		...
		it('should allow registered-users group to view diffs', async () => {
			...
			['timestamp', 'username'].every(property => Object.keys(data.revisions[0]).includes(property));
		});
	...
	describe('move', () => {
		...
		it('should error if data is invalid', async () => {
			try {
				await apiPosts.move({uid: globalModuleUid}, {});
			} catch (error) {
				return assert.equal(error.message, '[[error:invalid-data]]');
			}
		...
		it('should error if user does not have move privilege', async () => {
			try {
				...
				return assert.equal(error.message, '[[error:no-privileges]]');
			}
		...
		it('should move a post', async () => {
			await apiPosts.move({uid: globalModuleUid}, {pid: replyPid, tid: moveTid});
		...
		it('should fail to move post if not moderator of target category', async () => {
			...
			const result = await apiTopics.create({uid: globalModuleUid}, {title: 'target topic', content: 'queued topic', cid: cat2.cid});
			const moduleUid = await user.create({username: 'modofcat1'});
			const userPrivilegeList = await privileges.categories.getUserPrivilegeList();
			await privileges.categories.give(userPrivilegeList, cat1.cid, moduleUid);
			let error_;
			try {
				await apiPosts.move({uid: moduleUid}, {pid: replyPid, tid: result.tid});
			} catch (error) {
				error_ = error;
			}

			assert.strictEqual(error_.message, '[[error:no-privileges]]');
		});
	});

	describe('getPostSummaryByPids', () => {
		it('should return empty array for empty pids', done => {
			posts.getPostSummaryByPids([], 0, {}, (error, data) => {
				assert.ifError(error);
		...
		it('should get post summaries', done => {
			posts.getPostSummaryByPids([postData.pid], 0, {}, (error, data) => {
				assert.ifError(error);
				assert(data[0].user);
				assert(data[0].topic);
	...
	it('should get recent poster uids', done => {
		topics.reply({
			...
			assert.ifError(error);
			posts.getRecentPosterUids(0, 1, (error, uids) => {
				assert.ifError(error);
	...
	describe('parse', () => {
		it('should not crash and return falsy if post data is falsy', done => {
			posts.parsePost(null, (error, postData) => {
				assert.ifError(error);
		...
		it('should store post content in cache', done => {
			...
			posts.parsePost(postData, error => {
				assert.ifError(error);
				posts.parsePost(postData, error => {
					assert.ifError(error);
					global.env = oldValue;
		...
		it('should parse signature and remove links and images', done => {
			...
			posts.parseSignature(userData, 1, (error, data) => {
				assert.ifError(error);
	...
	describe('socket methods', () => {
		...
		it('should fail to get raw post because of privilege', done => {
			socketPosts.getRawPost({uid: 0}, pid, error => {
				assert.equal(error.message, '[[error:no-privileges]]');
				done();
			});
		});
		...
		it('should fail to get raw post because post is deleted', done => {
			posts.setPostField(pid, 'deleted', 1, error => {
				assert.ifError(error);
				socketPosts.getRawPost({uid: voterUid}, pid, error => {
					assert.equal(error.message, '[[error:no-post]]');
					done();
				});
			});
		});
		...
		it('should get raw post content', done => {
			posts.setPostField(pid, 'deleted', 0, error => {
				assert.ifError(error);
				socketPosts.getRawPost({uid: voterUid}, pid, (error, postContent) => {
					assert.ifError(error);
		...
		it('should get post category', done => {
			socketPosts.getCategory({uid: voterUid}, pid, (error, postCid) => {
				assert.ifError(error);
		...
		it('should error with invalid data', done => {
			socketPosts.getPidIndex({uid: voterUid}, null, error => {
				assert.equal(error.message, '[[error:invalid-data]]');
				done();
			});
		});
		...
		it('should get pid index', done => {
			socketPosts.getPidIndex({uid: voterUid}, {pid, tid: topicData.tid, topicPostSort: 'oldest_to_newest'}, (error, index) => {
				assert.ifError(error);
		...
		it('should get pid index in reverse', done => {
			topics.reply({
				...
				socketPosts.getPidIndex({uid: voterUid}, {pid: postData.pid, tid: topicData.tid, topicPostSort: 'newest_to_oldest'}, (error, index) => {
					assert.ifError(error);
	...
	describe('filterPidsByCid', () => {
		it('should return pids as is if cid is falsy', done => {
			posts.filterPidsByCid([1, 2, 3], null, (error, pids) => {
				assert.ifError(error);
		...
		it('should filter pids by single cid', done => {
			posts.filterPidsByCid([postData.pid, 100, 101], cid, (error, pids) => {
				assert.ifError(error);
		...
		it('should filter pids by multiple cids', done => {
			posts.filterPidsByCid([postData.pid, 100, 101], [cid, 2, 3], (error, pids) => {
				assert.ifError(error);
				assert.deepEqual([postData.pid], pids);
				done();
			});
	...
	describe('post queue', () => {
		...
		before(done => {
			...
			user.create({username: 'newuser'}, (error, _uid) => {
				assert.ifError(error);
				uid = _uid;
				done();
			});
		...
		it('should load queued posts', done => {
			helpers.loginUser('globalmod', 'globalmodpwd', (error, data) => {
				jar = data.jar;
				assert.ifError(error);
		...
		it('should error if data is invalid', done => {
			socketPosts.editQueuedContent({uid: globalModuleUid}, null, error => {
				assert.equal(error.message, '[[error:invalid-data]]');
				done();
			});
		});
		...
		it('should edit post in queue', done => {
			socketPosts.editQueuedContent({uid: globalModuleUid}, {id: queueId, content: 'newContent'}, error => {
				assert.ifError(error);
				request(`${nconf.get('url')}/api/post-queue`, {jar, json: true}, (error, res, body) => {
					assert.ifError(error);
					assert.equal(body.posts[1].type, 'reply');
		...
		it('should edit topic title in queue', done => {
			socketPosts.editQueuedContent({uid: globalModuleUid}, {id: topicQueueId, title: 'new topic title'}, error => {
				assert.ifError(error);
				request(`${nconf.get('url')}/api/post-queue`, {jar, json: true}, (error, res, body) => {
					assert.ifError(error);
					assert.equal(body.posts[0].type, 'topic');
		...
		it('should edit topic category in queue', done => {
			socketPosts.editQueuedContent({uid: globalModuleUid}, {id: topicQueueId, cid: 2}, error => {
				assert.ifError(error);
				request(`${nconf.get('url')}/api/post-queue`, {jar, json: true}, (error, res, body) => {
					assert.ifError(error);
					assert.equal(body.posts[0].type, 'topic');
					assert.equal(body.posts[0].data.cid, 2);
					socketPosts.editQueuedContent({uid: globalModuleUid}, {id: topicQueueId, cid}, error_ => {
		...
		it('should prevent regular users from approving posts', done => {
			socketPosts.accept({uid}, {id: queueId}, error => {
				assert.equal(error.message, '[[error:no-privileges]]');
				done();
			});
		});
		...
		it('should accept queued posts and submit', done => {
			...
			async.waterfall([
				...
				function (_ids, next) {
					ids = _ids;
					socketPosts.accept({uid: globalModuleUid}, {id: ids[0]}, next);
				},
				function (next) {
					socketPosts.accept({uid: globalModuleUid}, {id: ids[1]}, next);
				},
			], done);
		});

		it('should not crash if id does not exist', done => {
			socketPosts.reject({uid: globalModuleUid}, {id: '123123123'}, error => {
				assert.equal(error.message, '[[error:no-privileges]]');
				done();
			});
		});
		...
		it('should update queued post\'s topic if target topic is merged', async () => {
			...
			const result1 = await apiTopics.create({uid: globalModuleUid}, {title: 'topic A', content: 'topic A content', cid});
			const result2 = await apiTopics.create({uid: globalModuleUid}, {title: 'topic B', content: 'topic B content', cid});

			const result = await apiTopics.reply({uid}, {content: 'the moved queued post', tid: result1.tid});

			await topics.merge([
				result1.tid, result2.tid,
			], globalModuleUid, {mainTid: result2.tid});

			let postData = await posts.getQueuedPosts();
			postData = postData.filter(p => Number.parseInt(p.data.tid, 10) === Number.parseInt(result2.tid, 10));
	...
	describe('Topic Backlinks', () => {
		...
		describe('integration tests', () => {
			it('should create a topic event in the referenced topic', async () => {
				...
				assert.strictEqual(Number.parseInt(events[0].uid, 10), 1);
