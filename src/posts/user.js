...
module.exports = function (Posts) {
	Posts.getUserInfoForPosts = async function (uids, uid) {
		...
		const uidsSignatureSet = new Set(signatureUids.map(uid => Number.parseInt(uid, 10)));
		...
		const result = await Promise.all(userData.map(async userData => {
			...
			if (isMemberOfGroups && userData.groupTitleArray) {
				for (const [index, userGroup] of userData.groupTitleArray.entries()) {
	...
	async function getGroupsMap(userData) {
		const groupTitles = _.uniq(userData.flatMap(u => u && u.groupTitleArray));
		const groupsMap = {};
	...
	Posts.changeOwner = async function (pids, toUid) {
		...
		postData = postData.filter(p => p.pid && p.uid !== Number.parseInt(toUid, 10));
		...
		let repChange = 0;
		const postsByUser = {};
		for (const [i, post] of postData.entries()) {
			post.cid = cids[i];
			repChange += post.votes;
			bulkRemove.push([`uid:${post.uid}:posts`, post.pid], [`cid:${post.cid}:uid:${post.uid}:pids`, post.pid], [`cid:${post.cid}:uid:${post.uid}:pids:votes`, post.pid]);

			bulkAdd.push([`uid:${toUid}:posts`, post.timestamp, post.pid], [`cid:${post.cid}:uid:${toUid}:pids`, post.timestamp, post.pid]);
	...
	async function reduceCounters(postsByUser) {
		await async.eachOfSeries(postsByUser, async (posts, uid) => {
			const repChange = posts.reduce((accumulator, value) => accumulator + value.votes, 0);
			await Promise.all([
	...
	async function updateTopicPosters(postData, toUid) {
		const postsByTopic = _.groupBy(postData, p => Number.parseInt(p.tid, 10));
		await async.eachOf(postsByTopic, async (posts, tid) => {
			const postsByUser = _.groupBy(posts, p => Number.parseInt(p.uid, 10));
	...
	async function handleMainPidOwnerChange(postData, toUid) {
		...
		for (const post of mainPosts) {
			bulkRemove.push([`cid:${post.cid}:uid:${post.uid}:tids`, post.tid], [`uid:${post.uid}:topics`, post.tid]);

			bulkAdd.push([`cid:${post.cid}:uid:${toUid}:tids`, tidToTopic[post.tid].timestamp, post.tid], [`uid:${toUid}:topics`, tidToTopic[post.tid].timestamp, post.tid]);
			postsByUser[post.uid] = postsByUser[post.uid] || [];
			postsByUser[post.uid].push(post);
		}
