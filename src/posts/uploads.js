...
module.exports = function (Posts) {
	...
	Posts.uploads.listWithSizes = async function (pid) {
		...
		return sizes.map((sizeObject, index) => ({
			...sizeObject,
			name: paths[index],
		}));
	};
	...
	Posts.uploads.getUsage = async function (filePaths) {
		...
		const keys = filePaths.map(fileObject => `upload:${md5(fileObject.path.replace('-resized', ''))}:pids`);
	...
	Posts.uploads.dissociate = async function (pid, filePaths) {
		...
		if (!meta.config.preserveOrphanedUploads) {
			...
			const uploaderUids = (await db.getObjectsFields(deletePaths.map(path => `upload:${md5(path)}`))).map(o => (o ? o.uid || null : null));
			await Promise.all(uploaderUids.map((uid, index) => (
				uid && isFinite(uid) ? user.deleteUpload(uid, uid, deletePaths[index]) : null
			)).filter(Boolean));
	...
	Posts.uploads.deleteFromDisk = async filePaths => {
		if (typeof filePaths === 'string') {
			...
			throw new TypeError(`[[error:wrong-parameter-type, filePaths, ${typeof filePaths}, array]]`);
		}
	...
	Posts.uploads.saveSize = async filePaths => {
		...
		await Promise.all(filePaths.map(async fileName => {
			try {
				...
				winston.error(`[posts/uploads] Error while saving post upload sizes (${fileName}): ${error.message}`);
			}
		}));
	};
};
