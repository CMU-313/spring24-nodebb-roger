...
module.exports = function (Posts) {
	...
	function voteInProgress(pid, uid) {
		return Array.isArray(votesInProgress[uid]) && votesInProgress[uid].includes(Number.parseInt(pid, 10));
	}

	function putVoteInProgress(pid, uid) {
		votesInProgress[uid] = votesInProgress[uid] || [];
		votesInProgress[uid].push(Number.parseInt(pid, 10));
	}

	function clearVoteProgress(pid, uid) {
		if (Array.isArray(votesInProgress[uid])) {
			const index = votesInProgress[uid].indexOf(Number.parseInt(pid, 10));
			if (index !== -1) {
	...
	async function unvote(pid, uid, type, voteStatus) {
		...
		if (Number.parseInt(uid, 10) === Number.parseInt(owner, 10)) {
	...
	async function vote(type, unvote, pid, uid, voteStatus) {
		...
		await (type === 'upvote' && !unvote ? db.sortedSetAdd(`uid:${uid}:upvote`, now, pid) : db.sortedSetRemove(`uid:${uid}:upvote`, pid));

		await (type === 'upvote' || unvote ? db.sortedSetRemove(`uid:${uid}:downvote`, pid) : db.sortedSetAdd(`uid:${uid}:downvote`, now, pid));
	...
	async function adjustPostVotes(postData, uid, type, unvote) {
		...
		await (unvote ? db.setRemove(`pid:${postData.pid}:${type}`, uid) : db.setAdd(`pid:${postData.pid}:${type}`, uid));
	...
	Posts.updatePostVoteCount = async function (postData) {
		...
		if (threshold && postData.votes <= (-threshold)) {
			...
			const reportMessage = await translator.translate(`[[flags:auto-flagged, ${-postData.votes}]]`);
			const flagObject = await flags.create('post', postData.pid, adminUid, reportMessage, null, true);
			await flags.notify(flagObject, adminUid, true);
		}
	...
	async function updateTopicVoteCount(postData) {
		...
		if (postData.uid) {
			await (postData.votes === 0 ? db.sortedSetRemove(`cid:${topicData.cid}:uid:${postData.uid}:pids:votes`, postData.pid) : db.sortedSetAdd(`cid:${topicData.cid}:uid:${postData.uid}:pids:votes`, postData.votes, postData.pid));
		}

		if (Number.parseInt(topicData.mainPid, 10) !== Number.parseInt(postData.pid, 10)) {
