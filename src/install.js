...
function checkSetupFlagEnv() {
	...
	if (Object.keys(envConfigMap).some(key => envKeys.includes(key))) {
		winston.info('[install/checkSetupFlagEnv] checking env vars for setup info...');
		setupValue ||= {};

		for (const [eventName, eventValue] of Object.entries(process.env)) { // Get setup values from env
			if (eventName.startsWith('NODEBB_DB_')) {
				setupValue[`${process.env.NODEBB_DB}:${envConfigMap[eventName]}`] = eventValue;
			} else if (eventName.startsWith('NODEBB_')) {
				setupValue[envConfigMap[eventName]] = eventValue;
			}
		}

		setupValue['admin:password:confirm'] = setupValue['admin:password'];
	}
	...
	if (setupValue && typeof setupValue === 'object') {
		if (setupValue['admin:username'] && setupValue['admin:password'] && setupValue['admin:password:confirm'] && setupValue['admin:email']) {
			install.values = setupValue;
		} else {
			winston.error('[install/checkSetupFlagEnv] required values are missing for automated setup:');
			if (!setupValue['admin:username']) {
				winston.error('  admin:username');
			}

			if (!setupValue['admin:password']) {
				winston.error('  admin:password');
			}

			if (!setupValue['admin:password:confirm']) {
				winston.error('  admin:password:confirm');
			}

			if (!setupValue['admin:email']) {
...
async function completeConfigSetup(config) {
	...
	if (urlObject.port && (!install.values || !install.values.hasOwnProperty('port'))) {
...
async function createAdmin() {
	...
	async function success(results) {
		...
		try {
			...
			const [namespace, key] = error.message.slice(2, -2).split(':', 2);
			if (namespace && key && error.message.startsWith('[[') && error.message.endsWith(']]')) {
				const lang = require(path.join(__dirname, `../public/language/en-GB/${namespace}`));
				if (lang && lang[key]) {
					error.message = lang[key];
				}
			}

			winston.warn(`Password error, please try again. ${error.message}`);
			return await retryPassword(results);
		}
...
async function checkUpgrade() {
	...
	try {
		...
		if (error.message === 'schema-out-of-date') {
			await upgrade.run();
			return;
		}
...
install.save = async function (server_config) {
	...
	await fs.promises.writeFile(serverConfigPath, JSON.stringify({...currentConfig, ...server_config}, null, 4));
