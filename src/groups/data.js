...
module.exports = function (Groups) {
	Groups.getGroupsFields = async function (groupNames, fields) {
		...
		const ephemeralIndex = groupNames.reduce((memo, current, index) => {
			if (Groups.ephemeralGroups.includes(current)) {
				memo.push(index);
			}
		...
		if (ephemeralIndex.length > 0) {
			for (const index of ephemeralIndex) {
				groupData[index] = Groups.getEphemeralGroup(groupNames[index]);
			}
		}
