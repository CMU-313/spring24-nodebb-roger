...
module.exports = function (Groups) {
	Groups.update = async function (groupName, values) {
		...
		for (const property of ['userTitleEnabled', 'private', 'hidden', 'disableJoinRequests', 'disableLeave']) {
			if (values.hasOwnProperty(property) && typeof values[property] !== 'boolean') {
				values[property] = values[property] === 'true' || Number.parseInt(values[property], 10) === 1;
			}
		}
