...
module.exports = {
	...
	async method() {
		...
		for (const cid of cids) {
			...
			await privileges.categories.give(modulePrivileges.map(p => `groups:${p}`), cid, ['Global Moderators']);
		}

		await privileges.global.give(globalModulePrivs, 'Global Moderators');

		async function givePrivsToModerators(cid, groupPrefix) {
			const privGroups = modulePrivileges.map(priv => `cid:${cid}:privileges:${groupPrefix}${priv}`);
