...
module.exports = {
	...
	async method() {
		...
		const toDismiss = topicData.map((t, index) => (Number.parseInt(t.deleted, 10) === 1 ? pids[index] : null)).filter(Boolean);
...
async function dismissFlag(pid) {
	...
	if (Number.parseInt(postData.uid, 10) && Number.parseInt(postData.flags, 10) > 0) {
		await Promise.all([
