...
UserReset.validate = async function (code) {
	...
	return Number.parseInt(issueDate, 10) > Date.now() - twoHours;
};
...
UserReset.commit = async function (code, password) {
	...
	const ok = await Password.compare(password, userData.password, Boolean(Number.parseInt(userData['password:shaWrapped'], 10)));
	if (ok) {
...
UserReset.clean = async function () {
	...
	if (tokens.length === 0 && uids.length === 0) {
		return;
	}
...
UserReset.cleanByUid = async function (uid) {
	...
	await batch.processSortedSet('reset:issueDate', async tokens => {
		...
		for (const [code, result] of Object.entries(results)) {
			if (Number.parseInt(result, 10) === uid) {
