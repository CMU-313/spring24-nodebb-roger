...
module.exports = function (User) {
	...
	User.verifyInvitation = async function (query) {
		if (!query.token) {
			const error = meta.config.registrationType.startsWith('admin-') ? new Error('[[register:invite.error-admin-only]]') : new Error('[[register:invite.error-invite-only]]');
			throw error;
		}
	...
	User.joinGroupsFromInvitation = async function (uid, token) {
		...
		if (!groupsToJoin || groupsToJoin.length === 0) {
			return;
		}
