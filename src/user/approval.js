...
module.exports = function (User) {
	...
	async function sendNotificationToAdmins(username) {
		...
		await notifications.pushGroup(notificationObject, 'administrators');
	}
	...
	User.shouldQueueUser = async function (ip) {
		...
		if (registrationApprovalType === 'admin-approval-ip') {
	...
	User.getRegistrationQueue = async function (start, stop) {
		...
		await Promise.all(users.map(async user => {
			...
			user.customActions = [user.customActions].flat();
			/*
