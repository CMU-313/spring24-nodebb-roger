...
module.exports = function (User) {
	...
	async function onSettingsLoaded(uid, settings) {
		...
		settings.showemail = Number.parseInt(getSetting(settings, 'showemail', 0), 10) === 1;
		settings.showfullname = Number.parseInt(getSetting(settings, 'showfullname', 0), 10) === 1;
		settings.openOutgoingLinksInNewTab = Number.parseInt(getSetting(settings, 'openOutgoingLinksInNewTab', 0), 10) === 1;
		settings.dailyDigestFreq = getSetting(settings, 'dailyDigestFreq', 'off');
		settings.usePagination = Number.parseInt(getSetting(settings, 'usePagination', 0), 10) === 1;
		settings.topicsPerPage = Math.min(
			meta.config.maxTopicsPerPage,
			settings.topicsPerPage ? Number.parseInt(settings.topicsPerPage, 10) : defaultTopicsPerPage,
			defaultTopicsPerPage,
		);
		settings.postsPerPage = Math.min(
			meta.config.maxPostsPerPage,
			settings.postsPerPage ? Number.parseInt(settings.postsPerPage, 10) : defaultPostsPerPage,
			defaultPostsPerPage,
		);
		...
		settings.followTopicsOnCreate = Number.parseInt(getSetting(settings, 'followTopicsOnCreate', 1), 10) === 1;
		settings.followTopicsOnReply = Number.parseInt(getSetting(settings, 'followTopicsOnReply', 0), 10) === 1;
		settings.upvoteNotifFreq = getSetting(settings, 'upvoteNotifFreq', 'all');
		settings.restrictChat = Number.parseInt(getSetting(settings, 'restrictChat', 0), 10) === 1;
		settings.topicSearchEnabled = Number.parseInt(getSetting(settings, 'topicSearchEnabled', 0), 10) === 1;
		settings.updateUrlWithPostIndex = Number.parseInt(getSetting(settings, 'updateUrlWithPostIndex', 1), 10) === 1;
		settings.bootswatchSkin = validator.escape(String(settings.bootswatchSkin || ''));
		settings.homePageRoute = validator.escape(String(settings.homePageRoute || '')).replaceAll('&#x2F;', '/');
		settings.scrollToMyPost = Number.parseInt(getSetting(settings, 'scrollToMyPost', 1), 10) === 1;
	...
	function getSetting(settings, key, defaultValue) {
		...
		if (meta.config[key] || meta.config[key] === 0) {
	...
	User.saveSettings = async function (uid, data) {
		...
		const settings = {
			...
			topicsPerPage: Math.min(data.topicsPerPage, Number.parseInt(maxTopicsPerPage, 10) || 20),
			postsPerPage: Math.min(data.postsPerPage, Number.parseInt(maxPostsPerPage, 10) || 20),
