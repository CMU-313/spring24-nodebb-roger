...
Plugins.reloadRoutes = async function (parameters) {
	...
	await Plugins.hooks.fire('static:app.load', {
		app, router: parameters.router, middleware, controllers,
	});
...
Plugins.showInstalled = async function () {
	const directories = await fs.promises.readdir(Plugins.nodeModulesPath);

	let pluginPaths = await findNodeBBModules(directories);
...
async function findNodeBBModules(directories) {
	const pluginPaths = [];
