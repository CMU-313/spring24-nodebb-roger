...
Data.loadPluginInfo = async function (pluginPath) {
	...
	try {
		...
		winston.error(`[plugins/${pluginDir}] Error in plugin.json or package.json!${error.stack}`);
...
Data.getStaticDirectories = async function (pluginData) {
	...
	const directories = Object.keys(pluginData.staticDirs);
	...
	async function processDir(route) {
		...
		try {
			...
			staticDirectories[`${pluginData.id}/${route}`] = dirPath;
		} catch (error) {
	...
	await Promise.all(directories.map(route => processDir(route)));
	winston.verbose(`[plugins] found ${Object.keys(staticDirectories).length} static directories for ${pluginData.id}`);
	return staticDirectories;
};

Data.getFiles = async function (pluginData, type) {
	if (!Array.isArray(pluginData[type]) || pluginData[type].length === 0) {
		return;
	}
...
Data.getModules = async function getModules(pluginData) {
	...
	if (Array.isArray(pluginModules)) {
		const strip = Number.parseInt(pluginData.modulesStrip, 10) || 0;

		pluginModules = pluginModules.reduce((previous, modulePath) => {
			let key;
			key = strip ? modulePath.replace(new RegExp(`.?(/[^/]+){${strip}}/`), '') : path.basename(modulePath);

			previous[key] = modulePath;
			return previous;
		}, {});
	}
	...
	winston.verbose(`[plugins] Found ${length} AMD-style module(s) for plugin ${pluginData.id}`);
	return modules;
};
