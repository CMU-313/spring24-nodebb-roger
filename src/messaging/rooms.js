...
module.exports = function (Messaging) {
	...
	Messaging.canReply = async (roomId, uid) => {
		...
		const data = await plugins.hooks.fire('filter:messaging.canReply', {
			uid, roomId, inRoom, canReply: inRoom,
		});
	...
	Messaging.loadRoom = async (uid, data) => {
		...
		const [room, canReply, users, messages, isAdminOrGlobalModule] = await Promise.all([
		...
		room.isAdminOrGlobalMod = isAdminOrGlobalModule;
