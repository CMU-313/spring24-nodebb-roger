...
module.exports = function (Messaging) {
	...
	const canEditDelete = async (messageId, uid, type) => {
		...
		const isAdminOrGlobalModule = await user.isAdminOrGlobalMod(uid);

		if (meta.config.disableChat) {
			throw new Error('[[error:chat-disabled]]');
		} else if (!isAdminOrGlobalModule && meta.config.disableChatMessageEditing) {
		...
		if (isAdminOrGlobalModule && !messageData.system) {
			return;
		}
