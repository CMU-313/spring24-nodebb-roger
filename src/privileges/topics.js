...
privsTopics.get = async function (tid, uid) {
	...
	const isAdminOrModule = isAdministrator || isModerator;
	const editable = isAdminOrModule;
	...
	return await plugins.hooks.fire('filter:privileges.topics.get', {
		...
		editable,
		deletable,
		view_deleted: isAdminOrModule || isOwner || privData['posts:view_deleted'],
		view_scheduled: privData['topics:schedule'] || isAdministrator,
		isAdminOrMod: isAdminOrModule,
		disabled,
		tid,
		uid,
	});
...
privsTopics.isOwnerOrAdminOrMod = async function (tid, uid) {
	const [isOwner, isAdminOrModule] = await Promise.all([
		topics.isOwner(tid, uid),
		privsTopics.isAdminOrMod(tid, uid),
	]);
	return isOwner || isAdminOrModule;
};
