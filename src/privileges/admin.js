...
privsAdmin.list = async function (uid) {
	...
	if (!(await user.isAdministrator(uid))) {
		const index = Array.from(_privilegeMap.keys()).indexOf('admin:privileges');
		privilegeLabels.splice(index, 1);
		userPrivilegeList.splice(index, 1);
		groupPrivilegeList.splice(index, 1);
	}
