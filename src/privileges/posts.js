...
privsPosts.get = async function (pids, uid) {
	...
	const privileges = cids.map((cid, i) => {
		const isAdminOrModule = results.isAdmin || isModerator[cid];
...
privsPosts.canMove = async function (pid, uid) {
	...
	return await isAdminOrModule(pid, uid);
};
...
async function isAdminOrModule(pid, uid) {
