...
async function isModeratorsOfCategory(cid, uids) {
	...
	const isModerator = uids.map((uid, index) => check1[index] || check2[index] || check3[index]);
...
privsUsers.canEdit = async function (callerUid, uid) {
	...
	const [isAdmin, isGlobalModule, isTargetAdmin] = await Promise.all([
	...
	const data = await plugins.hooks.fire('filter:user.canEdit', {
		...
		isTargetAdmin,
		canEdit: isAdmin || (isGlobalModule && !isTargetAdmin),
		callerUid,
		uid,
	});
	return data.canEdit;
