...
async function sendQueueNotification(type, targetUid, path, notificationText) {
	...
	if (Number.parseInt(meta.config.postQueueNotificationUid, 10) > 0) {
		notificationData.from = meta.config.postQueueNotificationUid;
	}

	const notificationObject = await notifications.create(notificationData);
	await notifications.push(notificationObject, [targetUid]);
}
