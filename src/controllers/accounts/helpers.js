...
helpers.getUserDataByUserSlug = async function (userslug, callerUID, query = {}) {
	...
	const isSelf = Number.parseInt(callerUID, 10) === Number.parseInt(userData.uid, 10);
	...
	userData.groups = Array.isArray(results.groups) && results.groups.length > 0 ? results.groups[0] : [];
	...
	userData['email:confirmed'] = Boolean(userData['email:confirmed']);
	...
	userData.muted = Number.parseInt(userData.mutedUntil, 10) > Date.now();
	userData.website = escape(userData.website);
	userData.websiteLink = userData.website.startsWith('http') ? userData.website : `http://${userData.website}`;
