...
dashboardController.get = async function (request, res) {
	...
	res.render('admin/dashboard', {
		...
		notices,
		stats,
		canRestart: Boolean(process.send),
		lastrestart,
...
dashboardController.getAnalytics = async (request, res, next) => {
	// Basic validation
	const validUnits = ['days', 'hours'];
	const validSets = ['uniquevisitors', 'pageviews', 'pageviews:registered', 'pageviews:bot', 'pageviews:guest'];
	const until = request.query.until ? new Date(Number.parseInt(request.query.until, 10)) : Date.now();
	const count = request.query.count || (request.query.units === 'hours' ? 24 : 30);
	if (isNaN(until) || !validUnits.includes(request.query.units)) {
		return next(new Error('[[error:invalid-data]]'));
	}

	// Filter out invalid sets, if no sets, assume all sets
	let sets;
	if (request.query.sets) {
		sets = Array.isArray(request.query.sets) ? request.query.sets : [request.query.sets];
		sets = sets.filter(set => validSets.includes(set));
	} else {
		sets = validSets;
	}

	const method = request.query.units === 'days' ? analytics.getDailyStatsForSet : analytics.getHourlyStatsForSet;
...
async function getStatsFromAnalytics(set, field) {
	...
	const sum = array => array.reduce((memo, current) => memo + current, 0);
	const results = {
...
dashboardController.getLogins = async (request, res) => {
	...
	sessions = sessions.flat().sort((a, b) => b.datetime - a.datetime);
...
dashboardController.getUsers = async (request, res) => {
	let stats = await getStats();
	...
	const end = Number.parseInt(request.query.until, 10) || Date.now();
	const start = end - (1000 * 60 * 60 * (request.query.units === 'days' ? 24 : 1) * (request.query.count || (request.query.units === 'days' ? 30 : 24)));
...
dashboardController.getTopics = async (request, res) => {
	let stats = await getStats();
...
dashboardController.getSearches = async (request, res) => {
