...
mongoModule.questions = [
	...
	},
	{
		...
		ask: isUriNotSpecified,
		before(value) {
			value ||= nconf.get('mongo:password') || ''; return value;
		},
	},
	{
		name: 'mongo:database',
...
mongoModule.info = async function (database) {
	if (!database) {
		const client = await connection.connect(nconf.get('mongo'));
		database = client.db();
	}

	mongoModule.client = mongoModule.client || database;
	let serverStatusError = '';

	async function getServerStatus() {
		try {
			return await database.command({serverStatus: 1});
		} catch (error) {
			serverStatusError = error.message;
			// Override mongo error with more human-readable error
			if (error.name === 'MongoError' && error.codeName === 'Unauthorized') {
...
async function getCollectionStats(database) {
	const items = await database.listCollections().toArray();
	return await Promise.all(items.map(collection => database.collection(collection.name).stats()));
}

mongoModule.close = function (callback) {
	callback ||= function () {};
	client.close(error => callback(error));
};
