...
postgresModule.questions = [
	...
	},
	{
		...
		before(value) {
			value ||= nconf.get('postgres:password') || ''; return value;
		},
	},
	{
...
postgresModule.createSessionStore = async function (options) {
	...
	const database = await connection.connect(options);
...
postgresModule.info = async function (database) {
	database ||= await connection.connect(nconf.get('postgres'));

	postgresModule.pool = postgresModule.pool || database;
