...
SELECT h."data"->>$2::TEXT f
 ...
 LIMIT 1`,
		...
		return res.rows.length > 0 ? res.rows[0].f : null;
	};
...
SELECT ARRAY(SELECT jsonb_object_keys(h."data")) k
 ...
 LIMIT 1`,
		...
		return res.rows.length > 0 ? res.rows[0].k : [];
	};
...
SELECT (h."data" ? $2::TEXT AND h."data"->>$2::TEXT IS NOT NULL) b
 ...
 LIMIT 1`,
		...
		return res.rows.length > 0 ? res.rows[0].b : false;
	};
	...
	module.incrObjectFieldBy = async function (key, field, value) {
		...
		return await module.transaction(async client => {
			await (Array.isArray(key) ? helpers.ensureLegacyObjectsType(client, key, 'hash') : helpers.ensureLegacyObjectType(client, key, 'hash'));
...
RETURNING ("data"->>$2::TEXT)::NUMERIC v`,
			...
			return Array.isArray(key) ? res.rows.map(r => Number.parseFloat(r.v)) : Number.parseFloat(res.rows[0].v);
		});
	};
