...
module.exports = function (module) {
	...
	async function getSortedSetRange(key, start, stop, min, max, sort, withScores) {
		...
		if ((start < 0 && start > stop) || (isArray && key.length === 0)) {
			return [];
		}

		const query = {_key: key};
		if (isArray) {
			query._key = key.length > 1 ? {$in: key} : key[0];
		}
		...
		if (isArray && key.length > 100) {
			...
			if (start > 0) {
				result = result.slice(start, stop === -1 ? undefined : stop + 1);
			}
		} else {
	...
	async function sortedSetsRanks(method, keys, values) {
		...
		const data = Array.from({length: values.length});
		for (const [i, value] of values.entries()) {
			data[i] = {key: keys[i], value};
		}
	...
	module.isSortedSetMembers = async function (key, values) {
		...
		return values.map(value => Boolean(isMember[value]));
	};
	...
	module.isMemberOfSortedSets = async function (keys, value) {
		...
		return keys.map(key => Boolean(isMember[key]));
	};
	...
	module.sortedSetIncrBy = async function (key, increment, value) {
		...
		const data = {};
		value = helpers.valueToString(value);
		data.score = Number.parseFloat(increment);
	...
	module.sortedSetIncrByBulk = async function (data) {
		...
		for (const item of data) {
			bulk.find({_key: item[0], value: helpers.valueToString(item[2])})
				.upsert()
				.update({$inc: {score: Number.parseFloat(item[1])}});
		}
	...
	module.getSortedSetScan = async function (parameters) {
		...
		const match = helpers.buildMatchQuery(parameters.match);
		let regex;
		try {
		...
		const cursor = module.client.collection('objects').find({
			_key: parameters.key, value: {$regex: regex},
		}, {projection: project});
	...
	module.processSortedSet = async function (setKey, processFunction, options) {
		let done = false;
		const ids = [];
		...
		if (processFunction && processFunction.constructor && processFunction.constructor.name !== 'AsyncFunction') {
			processFunction = util.promisify(processFunction);
		}
