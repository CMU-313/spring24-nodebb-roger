...
module.exports = function (Categories) {
	...
	Categories.getTopicIds = async function (data) {
		...
		const pinnedTidsOnPage = pinnedTids.slice(data.start, data.stop === -1 ? undefined : data.stop + 1);
	...
	Categories.buildTopicsSortedSet = async function (data) {
		...
		if (data.tag) {
			set = Array.isArray(data.tag) ? [set].concat(data.tag.map(tag => `tag:${tag}:topics`)) : [set, `tag:${data.tag}:topics`];
		}
	...
	Categories.modifyTopicsByPrivilege = function (topics, privileges) {
		if (!Array.isArray(topics) || topics.length === 0 || privileges.view_deleted) {
			return;
		}

		for (const topic of topics) {
			if (!topic.scheduled && (topic.deleted || topic.private) && !topic.isOwner) {
				topic.title = topic.private ? '[[topic:topic_is_private]]' : '[[topic:topic_is_deleted]]';
