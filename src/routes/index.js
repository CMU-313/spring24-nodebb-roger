...
module.exports = async function (app, middleware) {
	...
	router.render = function (...arguments_) {
		app.render(...arguments_);
	};
	...
	for (const mount of remountable) {
...
function addCoreRoutes(app, router, middleware, mounts) {
	...
	for (const object of statics) {
		app.use(relativePath + object.route, middleware.addUploadHeaders, express.static(object.path, staticOptions));
	}

	app.use(`${relativePath}/uploads`, (request, res) => {
		res.redirect(`${relativePath}/assets/uploads${request.path}?${meta.config['cache-buster']}`);
	});
	app.use(`${relativePath}/plugins`, (request, res) => {
		winston.warn(`${chalk.bold.red('[deprecation]')} The \`/plugins\` shorthand prefix is deprecated, prefix with \`/assets/plugins\` instead (path: ${request.path})`);
		res.redirect(`${relativePath}/assets/plugins${request.path}${request._parsedUrl.search || ''}`);
	});
...
function addRemountableRoutes(app, router, middleware, mounts) {
	Object.keys(mounts).map(async mount => {
		...
		if (mount !== original) {
			...
			router.use(new RegExp(`/(api/)?${original}`), (request, res) => {
				controllerHelpers.redirect(res, `${nconf.get('relative_path')}/${mount}${request.path}`);
			});
		}
