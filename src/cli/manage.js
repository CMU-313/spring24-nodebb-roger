...
async function listPlugins() {
	...
	const combined = installed.concat(active.reduce((memo, current) => {
		if (!installedList.has(current)) {
			memo.push({
				id: current,
				active: true,
				installed: false,
...
async function info() {
	...
	for (const [index, point] of analyticsData.entries()) {
		graph.addPoint(index + 1, Math.round(point / max * 10));
	}
