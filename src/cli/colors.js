...
function humanReadableArgumentName(argument) {
...
module.exports = {
	commandUsage(cmd) {
		...
		const arguments_ = cmd._args.map(argument => chalk[colors[depth].arg](humanReadableArgumentName(argument)));
		const cmdUsage = [].concat(
			(cmd.options.length > 0 || cmd._hasHelpOption ? chalk[colors[depth].option]('[options]') : []),
			(cmd.commands.length > 0 ? chalk[colors[depth + 1].command]('[command]') : []),
			(cmd._args.length > 0 ? arguments_ : []),
		).join(' ');
	...
	subcommandTerm(cmd) {
		...
		const arguments_ = cmd._args.map(argument => humanReadableArgumentName(argument)).join(' ');
        ...
        + chalk[colors[depth].arg](arguments_ ? ` ${arguments_}` : '');
	},
